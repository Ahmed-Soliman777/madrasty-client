...
export default async function Home() {
    ...
    return (
        <section className="h-full min-h-screen bg-[#F8FAFC] text-[#0F172A] antialiased selection:bg-[#F5A623] selection:text-[#0A2540] font-sans">
                ...
                <div className="hidden lg:flex justify-between lg:w-[46%] xl:w-[50%] bg-linear-to-br from-[#0A2540] via-[#081f36] to-[#051524] text-white relative overflow-hidden flex-col p-12">
                    ...
                    <div className="relative z-10 flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <h3 className="text-base font-bold text-white tracking-wide">{t('schoolName')}</h3>
                        </div>
                    ...
                    <div className="relative z-10">
                        ...
                        <h2 className="text-3xl xl:text-4xl font-extrabold text-white leading-tight mb-4 tracking-tight">
                            {t('welcome')} <span className="text-transparent bg-clip-text bg-linear-to-l from-amber-300 via-amber-200 to-white">{t('brandName')}</span>
                        </h2>
                        ...
                        <div className="relative mx-auto lg:mx-0">
                            <div className="relative rounded-2xl bg-linear-to-b from-white/10 to-white/5 border border-white/15 backdrop-blur-md p-6 shadow-2xl overflow-hidden">
                                <div className="flex items-center justify-between pb-4 border-b border-white/10 mb-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <h4 className="text-xs font-bold text-white">{t('performanceTitle')}</h4>
                                            <p className="text-[10px] text-sky-200">{t('performanceUpdate')}</p>
                                        </div>
                                    </div>
                                    <span className="text-[11px] font-mono font-bold text-emerald-400 bg-emerald-950/60 px-2 py-0.5 rounded border border-emerald-500/30">{t('attendanceRate')}</span>
                                </div>

                                <div className="grid grid-cols-3 gap-2.5 mb-4">
                                    <div className="p-2.5 rounded-xl bg-white/5 border border-white/10 text-center">
                                        <span className="block text-[10px] text-slate-300">{t('achievementPoints')}</span>
                                        <span className="text-base font-extrabold text-amber-300 font-mono">+15</span>
                                    </div>
                                    <div className="p-2.5 rounded-xl bg-white/5 border border-white/10 text-center">
                                        <span className="block text-[10px] text-slate-300">{t('homework')}</span>
                                        <span className="text-base font-extrabold text-white font-mono">2 / 3</span>
                                    </div>
                                    <div className="p-2.5 rounded-xl bg-white/5 border border-white/10 text-center">
                                        <span className="block text-[10px] text-slate-300">{t('bus')}</span>
                            ...
                            <div className="absolute -top-3 -right-3 bg-linear-to-r from-[#F5A623] to-amber-500 text-[#0A2540] px-3 py-1.5 rounded-xl shadow-lg font-bold text-xs flex items-center gap-1.5 ring-2 ring-[#0A2540]">
                                ...
                                <span>{t('nileStar', { year: academicYear + 1 })}</span>
                            </div>
                    ...
                    <div className="relative z-10 pt-6 border-t border-white/10 flex items-center justify-between text-xs text-slate-400">
                        <span>{t('copyright', { year: academicYear })}</span>
                    </div>
